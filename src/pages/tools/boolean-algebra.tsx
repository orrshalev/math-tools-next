...
class BooleanAlgebra extends React.Component<any, State> {
  static eval(
    expression: string,
    ...
    const boolsLength: number = (bools.values()[0] as number[]).length;
          ...
          <thead className="border-b">
            <tr key="vars">
              {bools.keys().map((key: string) => (
                <th className={`border-r`} key={key}>{key}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {tran.map((row: any) => (
              <tr className="border-b" key={row.toString()}>
                {row.map((key: any) => (
                  <td className={`border-r`} key={key}>{key}</td>
                ))}
              </tr>
            ))}
          </tbody>
